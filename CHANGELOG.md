...
- **New toggle: `SprintPlan.artifact` — a visual sprint-plan page when `/new-sprint` finishes.** The Recap's front-half twin: one card per story in priority order (what it fixes, humanGated badges, dependencies), typographic, scannable in under a minute — scope gets approved from a page instead of raw prd.json. `/ralph-init` Question 4 is now "Visual artifact pages" and configures both moments at once (plan + recap / recap only / plan only / off); both write as `ask` mode, editable to `on`/`off` in RALPH.md. Falls back to an HTML file in `Audit.reports_dir` when the Artifact tool is unavailable, same as the Recap.
- **Reviewer model is chosen at init, not frozen at plugin release.** `/ralph-init`'s Reviewers follow-up now asks which model to pin the scaffolded Code Reviewer + QA Engineer to, building the options from the Claude lineup CURRENT at setup time (newest Sonnet tier recommended for verification-tier work; newest Opus tier for maximum scrutiny; or `inherit` to track the dispatching session's model). The chosen ID is rewritten into both copied agents' frontmatter; the shipped templates keep newest-Sonnet-at-release only as a fallback default. Rationale: a hardcoded model name in a template ages badly — the right reviewer model a year from now may be a Sonnet that doesn't exist yet, or something else entirely.
- **Auditor model is now config, same rationale: new `Audit.model` key in RALPH.md (default `sonnet`).** All four auditor spawn sites (`/start` per-story + sprint-close, `/audit`, `/new-sprint` PRD review) pass it as the Agent-tool `model` override, which takes precedence over the agent file's pin. Tier names (`sonnet` / `opus` / `haiku`) track the newest model in that tier automatically; a full model ID pins an exact version. Existing RALPH.md files without the key keep today's behavior (default `sonnet`).

## 2.3.0 — 2026-07-29

- **New workflow toggle: `Reviewers` — built-in PR review gate (Code Reviewer + QA Engineer).** Until now `Merge.mode = pr` opened the PR and told you to bring your own review. Opt in (asked at `/ralph-init` only when you pick `pr` — it has no effect in other modes) and init scaffolds two reviewer agents into the project's `.claude/agents/` plus a dispatch script at `scripts/ralph/pr-review.sh`. The Code Reviewer reviews HOW the code is written (security, performance, error handling, project conventions from CLAUDE.md); the QA Engineer reviews WHAT it delivers (every prd.json acceptance criterion, design-doc conformance, tracking-doc sync, test coverage, deployment readiness). Both run as parallel Claude Code CLI processes (Sonnet 5, pinned in frontmatter), post PR comments ending in APPROVE / REQUEST CHANGES verdicts, and `/start` Case D2 gates the merge on both APPROVEs **on the current head** — a verdict predating pushed fixes doesn't count. Re-review is the same command (agents diff their own prior comments: "Previously flagged X — RESOLVED / STILL PRESENT"); a single failed agent re-runs alone (`pr-review.sh <PR> code|qa`), and the flow explicitly verifies WHICH reviews actually posted before re-dispatching (field lesson from closetize-app PR #407: an agent can fail after posting — double-post — or fail without posting). The agent files land in the project, not the plugin, so checklists are per-project tunable; unedited they resolve project specifics from RALPH.md/CLAUDE.md at review time. Extracted and generalized from the pair battle-tested on closetize-app since 2026-05.
